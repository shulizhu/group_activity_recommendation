...
def get_tasks():
    ...
    return recommendActivity(my_list)


def recommendActivity(activities):
    dict = {}
    for x in activities:
        if x in dict:
            dict[x] += 1
    ...
    if len(res) >= 2 and dict[res[0]] == dict[res[-1]]:
        return 'The distribution of votes is too even, so a popular activity is recommended randomly: ' + randomRecommendation()
    else:
